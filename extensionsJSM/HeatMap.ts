 ...
 * @category Extension
 */
export enum HeatMapSpreadUnits {
  /**
...
// the bounding box (in canvas pixels) of all seeded pixels
// and the hottest (lowest) starting color index among them
interface SeedExtent {
  x0: number;
  y0: number;
  x1: number;
  y1: number;
...
export class HeatMap {
  ...
  private _metric: HeatMapMetric;
  private _spreadUnits: HeatMapSpreadUnits;
  private _chamferSize: number;
  private _updater: () => void;
  private _changer: (e: go.ChangedEvent) => void;
  // render caches, reused between updates to avoid per-update allocation
  private _field: Float32Array | null;
  ...
  set spreadUnits(value: HeatMapSpreadUnits) {
    if (
      this.spreadUnits !== value &&
  ...
  set isInForeground(value: boolean) {
    if (this.isInForeground !== value) {
      ...
      if (diag !== null) diag.remove(this.heatMapPart);
  ...
  private _renderHeatMap(
    ...
    if (
      ...
      if (reuse) this._imgdata = imgdata;
    }
    let seeded = false;
    ...
    const ext: SeedExtent = { x0: w, y0: h, x1: -1, y1: -1, minC: len1 };

    const parts = diag.findPartsIn(vb, true, false);
    parts.each((part) => {
      if (part instanceof go.Link) {
        if (this._seedLink(part, vb, w, h, sc, field, ext)) seeded = true;
      } else {
        if (this._seedPart(part, vb, w, h, sc, field, ext)) seeded = true;
      }
    });
      ...
      if (this._metric === HeatMapMetric.Euclidean) {
        ...
        for (let j = ry0; j <= ry1; j++) {
          ...
          for (let i = rx0; i <= rx1; i++) {
            const k = row + i;
            let v = field[k];
            if (i > 0 && field[k - 1] + step < v) v = field[k - 1] + step;
            if (j > 0) {
              if (field[k - w] + step < v) v = field[k - w] + step;
              if (i > 0 && field[k - w - 1] + diag < v) v = field[k - w - 1] + diag;
              if (i < w - 1 && field[k - w + 1] + diag < v) v = field[k - w + 1] + diag;
            }
            if (use5) {
              if (j > 0) {
                if (i > 1 && field[k - w - 2] + knight < v) v = field[k - w - 2] + knight;
                if (i < w - 2 && field[k - w + 2] + knight < v) v = field[k - w + 2] + knight;
              }
              if (j > 1) {
                if (i > 0 && field[k - 2 * w - 1] + knight < v) v = field[k - 2 * w - 1] + knight;
                if (i < w - 1 && field[k - 2 * w + 1] + knight < v) v = field[k - 2 * w + 1] + knight;
              }
            }
            field[k] = v;
          }
        ...
        for (let j = ry1; j >= ry0; j--) {
          ...
          for (let i = rx1; i >= rx0; i--) {
            const k = row + i;
            let v = field[k];
            if (i < w - 1 && field[k + 1] + step < v) v = field[k + 1] + step;
            if (j < h - 1) {
              if (field[k + w] + step < v) v = field[k + w] + step;
              if (i < w - 1 && field[k + w + 1] + diag < v) v = field[k + w + 1] + diag;
              if (i > 0 && field[k + w - 1] + diag < v) v = field[k + w - 1] + diag;
        ...
        for (let j = ry0; j <= ry1; j++) {
          ...
          for (let i = rx0; i <= rx1; i++) {
            ...
            let v = field[k];
            if (i > 0 && field[k - 1] + step < v) v = field[k - 1] + step;
            if (j > 0 && field[k - w] + step < v) v = field[k - w] + step;
            field[k] = v;
          }
        }
        ...
        for (let j = ry1; j >= ry0; j--) {
          ...
          for (let i = rx1; i >= rx0; i--) {
            ...
            let v = field[k];
            if (i < w - 1 && field[k + 1] + step < v) v = field[k + 1] + step;
            if (j < h - 1 && field[k + w] + step < v) v = field[k + w] + step;
            field[k] = v;
          }
        }
      }
  ...
  private _extendExtent(
    ...
    if (x0 < ext.x0) ext.x0 = x0;
    if (y0 < ext.y0) ext.y0 = y0;
    if (x1 > ext.x1) ext.x1 = x1;
    if (y1 > ext.y1) ext.y1 = y1;
    if (startC < ext.minC) ext.minC = startC;
  }
  ...
  private _seedLink(
    ...
    if (part.pointsCount < 2) return false;
    const b = part.routeBounds.copy();
    if (!b.intersectsRect(vb)) return false;
    ...
    if (part.computeCurve() === go.Curve.Bezier) {
      for (let i = 0; i < part.pointsCount - 1; i += 3) {
        ...
        for (let t = 0; t <= 1; t += 1 / pix) {
          ...
          const k = py * w + px;
          if (startC < field[k]) field[k] = startC;
        }
      }
    } else {
      ...
      for (let i = 1; i < part.pointsCount; i++) {
        ...
        for (let z = 0; z < am; z++) {
          ...
          const k2 = y2 * w + x2;
          if (startC < field[k2]) field[k2] = startC;
        }
        vp = vq;
      }
    }
      ...
      for (let y = 1; y <= ry; y++) {
        ...
        for (let x = -x0; x <= x0; x++) {
          if (oy - y >= 0 && oy - y < h && ox + x >= 0 && ox + x < w) {
            ...
            if (startC < field[km]) field[km] = startC;
          }
          if (oy + y >= 0 && oy + y < h && ox + x >= 0 && ox + x < w) {
            const kp = (oy + y) * w + (ox + x);
            if (startC < field[kp]) field[kp] = startC;
          }
        }
      }
    } else {
