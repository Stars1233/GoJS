 ...
 */
export class CroppingTool extends go.ResizingTool {
  private _croppedObjectName: string;
  private _minViewSize: number;
  private _aspectRatio: number | null;
  private _croppingPart: go.Part;
  private _nodeBeingCropped: go.Part | null;
  private _originalSourceRect: go.Rect;
  private _startPoint: go.Point; // mouse-down point in full-image coordinates
  private _viewportListener: (e: go.DiagramEvent) => void;
  private _baseFocus: WeakMap<go.GraphObject, go.Spot>; // handles unscaled alignmentFocus

  constructor(init?: Partial<CroppingTool>) {
    super();
    this.name = 'Cropping';
    ...
    this._croppingPart = new go.Part({
      ...
      selectionChanged: (part) => {
        if (!part.isSelected) this.stopCropping();
      },
      copyable: false,
      deletable: false,
      movable: false,
      ...
      new go.Panel('Spot').add(
        ...
        new go.Shape({ fill: '#888888A0', strokeWidth: 0, stretch: go.Stretch.Fill })
      ),
      new go.Picture({
        name: 'CROPPED',
        imageStretch: go.ImageStretch.None,
   ...
   */
  get croppedObjectName(): string {
    return this._croppedObjectName;
  }
  set croppedObjectName(val: string) {
  ...
  set aspectRatio(val: number | null) {
   ...
   */
  get nodeBeingCropped(): go.Part | null {
   ...
   */
  startCropping(part?: go.Part): void {
    const diagram = this.diagram;
    if (diagram === null || diagram.isReadOnly) return;
    if (this._nodeBeingCropped !== null) return; // already cropping
    let node = part;
    if (node === undefined) {
    ...
    if (!(pic instanceof go.Picture)) return;
    ...
    if (!(full instanceof go.Picture) || !(cropped instanceof go.Picture)) return;
    this._nodeBeingCropped = node;
    // set up and show the croppingPart
    diagram.commit((diag) => {
      const cb = pic.sourceRect.copy();
      if (!cb.isReal()) cb.set(pic.naturalBounds); // no sourceRect means the whole image
      ...
      if (natw > 0 && nath > 0) {
        ...
        if (vb.isReal()) {
          ...
          if (isFinite(fit) && fit > 0) cs = Math.min(cs, fit);
        }
      }
  ...
  stopCropping(apply?: boolean): void {
    ...
    const full = croppingPart.findObject('FULL');
    const cropped = croppingPart.findObject('CROPPED');
    if (diagram === null || !(full instanceof go.Picture) || !(cropped instanceof go.Picture)) return;
   ...
   */
  override makeAdornment(cropObj: go.GraphObject): go.Adornment {
  ...
  override updateAdornments(part: go.Part): void {
    ...
    if (part === this._croppingPart && part.isSelected && !this.diagram.isReadOnly) {
      ...
      if (
        ...
        if (box instanceof go.Shape) box.strokeWidth = hs / box.scale; // one pixel on screen
  ...
  override doActivate(): void {
    ...
    if (!(cropObj instanceof go.Picture)) return;
    this._originalSourceRect = cropObj.sourceRect.copy();
    if (!this._originalSourceRect.isReal()) this._originalSourceRect.set(cropObj.naturalBounds);
  }
  ...
  override doCancel(): void {
    if (this.adornedObject !== null) this.resize(this._originalSourceRect);
    this.stopTool();
  }
  ...
  override computeResize(
    ...
    const obj = this.adornedObject;
    if (obj === null || obj.panel === null) return b;
    ...
    if (spot.x <= 0 || spot.x >= 1) {
      ...
      if (spot.y <= 0) w = Math.max(w, (fy - pt.y) * ratio);
      else if (spot.y >= 1) w = Math.max(w, (pt.y - fy) * ratio);
