 ...
 */
class SwimLaneCompactLayout extends go.Layout {
    constructor(init) {
        super();
    ...
    set laneProperty(val) {
        if (typeof val !== 'string' && typeof val !== 'function')
            throw new Error('new value for SwimLaneCompactLayout.laneProperty must be a property name, not: ' + val);
    ...
    set laneComparer(val) {
        if (val !== null && typeof val !== 'function')
            throw new Error('new value for SwimLaneCompactLayout.laneComparer must be a function or null, not: ' + val);
     ...
     */
    get maxCycleBlockSize() {
        return this._maxCycleBlockSize;
    }
    set maxCycleBlockSize(val) {
        if (typeof val !== 'number' || isNaN(val) || val < 0)
            throw new Error('new value for SwimLaneCompactLayout.maxCycleBlockSize must be a non-negative number, not: ' +
                val);
    ...
    set direction(val) {
        if (val !== 0 && val !== 90 && val !== 180 && val !== 270)
            throw new Error('new value for SwimLaneCompactLayout.direction must be 0, 90, 180, or 270, not: ' + val);
    ...
    set layerSpacing(val) {
        if (typeof val !== 'number' || isNaN(val) || val < 0)
            throw new Error('new value for SwimLaneCompactLayout.layerSpacing must be a non-negative number, not: ' + val);
    ...
    set laneSpacing(val) {
        if (typeof val !== 'number' || isNaN(val) || val < 0)
            throw new Error('new value for SwimLaneCompactLayout.laneSpacing must be a non-negative number, not: ' + val);
    ...
    set lanePadding(val) {
        if (typeof val !== 'number' || isNaN(val) || val < 0)
            throw new Error('new value for SwimLaneCompactLayout.lanePadding must be a non-negative number, not: ' + val);
    ...
    set minLaneHeight(val) {
        if (typeof val !== 'number' || isNaN(val) || val < 0)
            throw new Error('new value for SwimLaneCompactLayout.minLaneHeight must be a non-negative number, not: ' + val);
    ...
    set minColumnWidth(val) {
        if (typeof val !== 'number' || isNaN(val) || val < 0)
            throw new Error('new value for SwimLaneCompactLayout.minColumnWidth must be a non-negative number, not: ' + val);
    ...
    set channelSpacing(val) {
        if (typeof val !== 'number' || isNaN(val) || val < 0)
            throw new Error('new value for SwimLaneCompactLayout.channelSpacing must be a non-negative number, not: ' + val);
    ...
    collectGraph(coll) {
        ...
        if (typeof this.laneComparer === 'function')
            laneNames.sort(this.laneComparer);
        ...
        nodes.forEach((n) => {
            var _a;
            return this._info.set(n, {
                node: n,
                row: (_a = laneRow.get(laneOf.get(n))) !== null && _a !== void 0 ? _a : 0,
                col: -1,
                lcol: -1,
                idx: 0,
                indeg: 0,
     ...
     */
    layoutBlock(members) {
        if (members.length <= 6 && this.searchBlockLayout(members))
            return;
    ...
    greedyBlockCols(members) {
        ...
        while (lorder.length < members.length) {
            let ready = lrem.filter((m) => { var _a; return ((_a = localIndeg.get(m)) !== null && _a !== void 0 ? _a : 0) <= 0; });
            if (ready.length === 0) {
                lrem.sort((a, b) => {
                    var _a, _b;
                    const d = ((_a = localIndeg.get(a)) !== null && _a !== void 0 ? _a : 0) - ((_b = localIndeg.get(b)) !== null && _b !== void 0 ? _b : 0);
                    if (d !== 0)
                        return d;
            ...
            m.succs.forEach((s) => {
                var _a;
                if (mset.has(s))
                    localIndeg.set(s, ((_a = localIndeg.get(s)) !== null && _a !== void 0 ? _a : 0) - 1);
            });
        }
        ...
        lorder.forEach((m) => {
            ...
            while (occ.has(this.cellKey(m.row, lc)))
                lc++;
            assigned.set(m, lc);
    ...
    searchBlockLayout(members) {
        ...
        if (entries.length === 0)
            entries = members.filter((m) => m.succs.some((s) => !mset.has(s)));
        if (entries.length === 0)
            entries = [members[0]];
        const depth = new Map();
        ...
        while (bfs.length > 0) {
            ...
            cur2.succs.forEach((s) => {
                var _a;
                if (!mset.has(s) || depth.has(s))
                    return;
                depth.set(s, ((_a = depth.get(cur2)) !== null && _a !== void 0 ? _a : 0) + 1);
                bfs.push(s);
            });
        }
        const edgeFlow = edges.map((e2) => { var _a, _b; return ((_a = depth.get(members[e2[1]])) !== null && _a !== void 0 ? _a : 0) > ((_b = depth.get(members[e2[0]])) !== null && _b !== void 0 ? _b : 0); });
        ...
        const lanePop = new Map();
        rows.forEach((r2) => { var _a; return lanePop.set(r2, ((_a = lanePop.get(r2)) !== null && _a !== void 0 ? _a : 0) + 1); });
        let floorScore = 0;
        let maxPerLane = 1;
        lanePop.forEach((cnt) => {
            if (cnt > maxPerLane)
        ...
        const hB = [];
        const vB = [];
        function shapeCost(fr, fc, tr, tc, shape, h, v) {
            var _a, _b;
            h.length = 0;
            v.length = 0;
            let cc, rr, b;
            ...
            if (fr === tr) {
                ...
                for (rr = lo + 1; rr < hi; rr++) {
                    if (occ.has(rr * BPK + fc))
                        return -1;
                }
                for (b = lo; b < hi; b++)
                    v.push(fc * BPK + b);
                for (cc = fc; cc < tc; cc++) {
                    if (occ.has(tr * BPK + cc))
                        return -1;
                    h.push(tr * BPK + cc);
                }
            }
            let cost = 0;
            for (let k2 = 0; k2 < h.length; k2++)
                cost += (_a = hSet.get(h[k2])) !== null && _a !== void 0 ? _a : 0;
            for (let k2 = 0; k2 < v.length; k2++)
                cost += (_b = vSet.get(v[k2])) !== null && _b !== void 0 ? _b : 0;
            return cost;
        }
        const cols = new Array(n).fill(0);
        let bestScore = Infinity;
        let bestCols = null;
        function scoreAssignment(limit) {
            var _a, _b;
            occ.clear();
            hSet.clear();
            vSet.clear();
            ...
            for (let ei = 0; ei < edges.length; ei++) {
                ...
                if (a < 0 && b2 < 0) {
                    ...
                    const cv = useB ? vB : vA;
                    total += useB ? b2 : a;
                    for (let k2 = 0; k2 < ch.length; k2++)
                        hSet.set(ch[k2], ((_a = hSet.get(ch[k2])) !== null && _a !== void 0 ? _a : 0) + 1);
                    for (let k2 = 0; k2 < cv.length; k2++)
                        vSet.set(cv[k2], ((_b = vSet.get(cv[k2])) !== null && _b !== void 0 ? _b : 0) + 1);
                }
                if (violations * 100000 + flowBad * 5000 + total * 1000 >= limit)
                    return Infinity;
            }
            let width = 0;
            let colSum = 0;
        ...
        const laneMasks = [];
        let done = false;
        function rec(i, flowPartial, colSum, maxColSoFar, usedZero) {
            if (done)
                return;
            if (flowPartial * 5000 + (maxColSoFar + 1) * 50 + colSum >= bestScore)
                return;
            if (i === n) {
            ...
            for (let c2 = 0; c2 < n; c2++) {
                ...
                for (let k2 = 0; k2 < closing.length; k2++) {
                    const ei = closing[k2];
                    if (edgeFlow[ei] && cols[edges[ei][1]] < cols[edges[ei][0]])
                        fp++;
                }
    ...
    buildComponents(compOf) {
        ...
        sccsRaw.forEach((members) => {
            if (members.length > this.maxCycleBlockSize)
                members.forEach((m) => sccs.push([m]));
            else
                sccs.push(members);
        });
        const comps = sccs.map((members) => {
            const comp = {
                members: members,
                preds: [],
                succs: [],
        ...
        comps.forEach((comp) => {
            if (comp.members.length > 1)
                this.layoutBlock(comp.members);
            else
                comp.members[0].lcol = 0;
            comp.members.forEach((m) => {
                if (m.row < comp.rowMin)
                    comp.rowMin = m.row;
                if (m.row > comp.rowMax)
                    comp.rowMax = m.row;
                if (m.lcol + 1 > comp.width)
                    comp.width = m.lcol + 1;
                if (m.idx < comp.minIdx)
                    comp.minIdx = m.idx;
            });
        });
    ...
    orderComponents(comps) {
        ...
        function promoteReady() {
            const still = [];
            const batch = [];
            remaining.forEach((c) => { var _a; return (((_a = indeg.get(c)) !== null && _a !== void 0 ? _a : 0) <= 0 ? batch : still).push(c); });
            remaining = still;
        ...
        while (order.length + loose.length < comps.length) {
            if (queue.length === 0) {
                remaining.sort((a, b) => { var _a, _b; return ((_a = indeg.get(a)) !== null && _a !== void 0 ? _a : 0) - ((_b = indeg.get(b)) !== null && _b !== void 0 ? _b : 0) || a.minIdx - b.minIdx; });
                queue.push(remaining.shift());
            }
            const cur = queue.shift();
            order.push(cur);
            cur.succs.forEach((s) => { var _a; return indeg.set(s, ((_a = indeg.get(s)) !== null && _a !== void 0 ? _a : 0) - 1); });
            promoteReady();
        }
     ...
     */
    tryRoute(pr, pc, r, c, shape, extraH, extraV) {
        ...
        function hRun(row, c1, c2) {
            ...
            for (let x = c1; x <= c2; x++) {
                ...
                hCells.push(key);
                cost += ((_a = hUse.get(key)) !== null && _a !== void 0 ? _a : 0) + (extraH.has(key) ? 1 : 0);
            }
            return true;
        }
        function vRun(col, r1, r2) {
            var _a;
            ...
            for (let b = lo; b < hi; b++) {
                ...
                vSegs.push(key);
                cost += ((_a = vUse.get(key)) !== null && _a !== void 0 ? _a : 0) + (extraV.has(key) ? 1 : 0);
            }
            return true;
        }
        function done(shp) {
        ...
        if (pr === r) {
            if (shape === 'H') {
                if (!hRun(pr, Math.min(pc, c) + 1, Math.max(pc, c) - 1))
                    return null;
                return done('flat');
            }
            if (shape === 'U' || shape === 'D') {
                ...
                if (!vRun(pc, pr, dr))
                    return null;
                if (!hRun(dr, Math.min(pc, c), Math.max(pc, c)))
                    return null;
                if (!vRun(c, dr, r))
                    return null;
        ...
        const forward = c > pc;
        if (shape === 'H') {
            if (!hRun(pr, forward ? pc + 1 : c, forward ? c : pc - 1))
                return null;
            if (!vRun(c, pr, r))
                return null;
        ...
        if (shape === 'V') {
            if (!vRun(pc, pr, r))
                return null;
            if (!hRun(r, forward ? pc : c + 1, forward ? c - 1 : pc))
                return null;
     ...
     */
    bestRoute(pr, pc, r, c, extraH, extraV) {
        const shapes = pr === r
        ...
        for (let i = 0; i < shapes.length; i++) {
            ...
            if (rt !== null && (pick === null || rt.cost < pick.cost))
                pick = rt;
        }
        return pick;
    }
     ...
     */
    exitSideOf(shape, fi, ti) {
        if (shape === 'flat')
            return ti.col > fi.col ? 'R' : 'L';
        if (shape === 'flatU')
            return 'T';
        if (shape === 'flatD')
            return 'B';
    ...
    claimRoute(rt, fromInfo, toInfo) {
        ...
        rt.hCells.forEach((cell) => {
            ...
            if (grid[cr][cc] === undefined)
                grid[cr][cc] = 2;
            this._hUse.set(cell, ((_a = this._hUse.get(cell)) !== null && _a !== void 0 ? _a : 0) + 1);
        });
        ...
        rt.vSegs.forEach((s) => { var _a; return this._vUse.set(s, ((_a = this._vUse.get(s)) !== null && _a !== void 0 ? _a : 0) + 1); });
     ...
     */
    unclaimRoute(rt, fromInfo, toInfo) {
        rt.hCells.forEach((cell) => { var _a; return this._hUse.set(cell, ((_a = this._hUse.get(cell)) !== null && _a !== void 0 ? _a : 1) - 1); });
        rt.vSegs.forEach((s) => { var _a; return this._vUse.set(s, ((_a = this._vUse.get(s)) !== null && _a !== void 0 ? _a : 1) - 1); });
        const st = this._sideOut.get(String(fromInfo.node.key) + '|' + this.exitSideOf(rt.shape, fromInfo, toInfo));
        if (st)
            st.delete(String(toInfo.node.key));
    }
    /**
     * @hidden @internal
     ...
     */
    placeComponents(order, compOf) {
        const grid = this._grid;
        order.forEach((comp) => {
            const predEdges = [];
            comp.members.forEach((m) => {
                m.preds.forEach((p) => {
                    if (compOf.get(p) !== comp && p.col >= 0)
                        predEdges.push({ p: p, m: m });
                });
            });
            let minBase = 0;
            predEdges.forEach((e) => {
            ...
            for (let guard = 0; guard < 10000; guard++, base++) {
                ...
                for (let mr = comp.rowMin; mr <= comp.rowMax; mr++) {
                    for (let mw = 0; mw < comp.width; mw++) {
                        if (grid[mr][base + mw] === undefined)
                            grid[mr][base + mw] = 3;
                    }
                }
                const routes = [];
                let violations = 0;
                const extraH = new Set();
                const extraV = new Set();
                const bb = base;
                ...
                for (let ur = comp.rowMin; ur <= comp.rowMax; ur++) {
                    for (let uw = 0; uw < comp.width; uw++) {
                        if (grid[ur][base + uw] === 1 || grid[ur][base + uw] === 3)
                            grid[ur][base + uw] = undefined;
                    }
                }
                if (best === null || violations < best.violations) {
                    best = { base: base, violations: violations, routes: routes };
                }
                if (best.violations === 0 || base > minBase + this._maxCol + this._infos.length + 2)
                    break;
            }
            if (best === null)
                best = { base: base, violations: 0, routes: [] };
            const chosenBase = best.base;
            comp.members.forEach((m) => {
                m.col = chosenBase + m.lcol;
                grid[m.row][m.col] = 1;
                if (m.col > this._maxCol)
                    this._maxCol = m.col;
            });
            for (let rr2 = comp.rowMin; rr2 <= comp.rowMax; rr2++) {
                for (let ww2 = 0; ww2 < comp.width; ww2++) {
                    if (grid[rr2][chosenBase + ww2] === undefined)
                        grid[rr2][chosenBase + ww2] = 3;
                }
            }
            ...
            if (comp.members.length > 1) {
                ...
                comp.members.forEach((m) => {
                    m.succs.forEach((s) => {
                        if (!mset.has(s))
                            return;
                        if (this._edgeShape.has(this.edgeKeyOf(m, s)))
                            return;
                        const rt2 = this.bestRoute(m.row, m.col, s.row, s.col, SwimLaneCompactLayout.NO_EXTRA, SwimLaneCompactLayout.NO_EXTRA);
                        if (rt2 !== null)
                            this.claimRoute(rt2, m, s);
                    });
                });
            }
        });
     ...
     * left open.
     */
    routeRemainingLinks(links) {
        links.forEach((l) => {
            const lf = this.infoFor(l.fromNode);
            const lt = this.infoFor(l.toNode);
            if (!lf || !lt || lf === lt)
                return;
            if (this._edgeShape.has(this.edgeKeyOf(lf, lt)))
                return;
            const lrt = this.bestRoute(lf.row, lf.col, lt.row, lt.col, SwimLaneCompactLayout.NO_EXTRA, SwimLaneCompactLayout.NO_EXTRA);
            if (lrt !== null)
                this.claimRoute(lrt, lf, lt);
        });
    }
    /**
     * @hidden @internal
    ...
    rerouteEdge(fi, ti) {
        ...
        for (let si = 0; si < shapes.length; si++) {
            const rt2 = this.tryRoute(fi.row, fi.col, ti.row, ti.col, shapes[si], SwimLaneCompactLayout.NO_EXTRA, SwimLaneCompactLayout.NO_EXTRA);
            if (rt2 === null)
                continue;
            const others = this._sideOut.get(String(fi.node.key) + '|' + this.exitSideOf(rt2.shape, fi, ti));
            if (others) {
                others.forEach((t2) => {
                    if (t2 !== String(ti.node.key))
                        rt2.cost += 2;
                });
            }
            if (pick === null || rt2.cost < pick.cost)
                pick = rt2;
        }
        return pick;
    }
    ...
    refineRoutes(links) {
        for (let sweep = 0; sweep < 2; sweep++) {
            ...
            links.forEach((l) => {
                ...
                if (next !== null && next.shape !== old.shape)
                    changed = true;
    ...
    mapSpot(s) {
        ...
        if (dir === 0)
            return s;
        if (dir === 180) {
            if (s.equals(go.Spot.Right))
                return go.Spot.Left;
            if (s.equals(go.Spot.Left))
                return go.Spot.Right;
            return s;
        }
        if (s.equals(go.Spot.Right))
            return dir === 90 ? go.Spot.Bottom : go.Spot.Top;
        if (s.equals(go.Spot.Left))
            return dir === 90 ? go.Spot.Top : go.Spot.Bottom;
        if (s.equals(go.Spot.Top))
            return go.Spot.Left;
        return go.Spot.Right;
    }
    /**
    ...
    commitResults(diagram, links) {
        ...
        if (this.setsPortSpots)
            this.assignLinkSpots(links);
        if (this.setsRoutePoints)
            this.assignRoutePoints(links, colU, colExtent, laneV, laneBreadth, totalU);
        ...
        lanes.forEach((lane, i) => {
            ...
            if (horiz) {
                if (ph !== null)
                    ph.desiredSize = new go.Size(bandLen, laneBreadth[i]);
                group.location = new go.Point(origin.x + bandStart, origin.y + laneV[i]);
            }
            else {
                if (ph !== null)
                    ph.desiredSize = new go.Size(laneBreadth[i], bandLen);
     ...
     */
    assignRoutePoints(links, colU, colExtent, laneV, laneBreadth, totalU) {
        const uOf = (c2) => colU[c2] + colExtent[c2] / 2;
        const vOf = (r2) => laneV[r2] + laneBreadth[r2] / 2;
        const hRunsByRow = new Map();
        const vRunsByCol = new Map();
        function regRun(map, mkey, a, b2, maxOff) {
            const run = {
                lo: Math.min(a, b2),
                hi: Math.max(a, b2),
                off: 0,
        ...
        links.forEach((l) => {
            ...
            const plan = { link: l, shape: shape, fi: fi, ti: ti, h: null, v: null, v2: null };
            const down = ti.row > fi.row;
            if (shape === 'flat') {
                const fwd = ti.col > fi.col;
                plan.h = regRun(hRunsByRow, fi.row, fwd ? fi.u1 : ti.u1, fwd ? ti.u0 : fi.u0, Math.min(fi.v1 - fi.v0, ti.v1 - ti.v0) / 2 - 5);
            }
            else if (shape === 'vert') {
                plan.v = regRun(vRunsByCol, ti.col, down ? fi.v1 : ti.v1, down ? ti.v0 : fi.v0, Math.min(fi.u1 - fi.u0, ti.u1 - ti.u0) / 2 - 5);
            }
            else if (shape === 'H' || shape === 'HL') {
                plan.h = regRun(hRunsByRow, fi.row, shape === 'H' ? fi.u1 : fi.u0, uOf(ti.col), (fi.v1 - fi.v0) / 2 - 5);
                plan.v = regRun(vRunsByCol, ti.col, vOf(fi.row), down ? ti.v0 : ti.v1, (ti.u1 - ti.u0) / 2 - 5);
            }
            else if (shape === 'V' || shape === 'VL') {
                plan.v = regRun(vRunsByCol, fi.col, down ? fi.v1 : fi.v0, vOf(ti.row), (fi.u1 - fi.u0) / 2 - 5);
                plan.h = regRun(hRunsByRow, ti.row, uOf(fi.col), shape === 'V' ? ti.u0 : ti.u1, (ti.v1 - ti.v0) / 2 - 5);
            }
            else if (shape === 'flatU' || shape === 'flatD') {
                const dr = shape === 'flatU' ? fi.row - 1 : fi.row + 1;
                const vd = vOf(dr);
                plan.v = regRun(vRunsByCol, fi.col, shape === 'flatU' ? fi.v0 : fi.v1, vd, (fi.u1 - fi.u0) / 2 - 5);
                plan.v2 = regRun(vRunsByCol, ti.col, vd, shape === 'flatU' ? ti.v0 : ti.v1, (ti.u1 - ti.u0) / 2 - 5);
        ...
        function assignChannelOffsets(map) {
            map.forEach((runs) => {
...
SwimLaneCompactLayout.PK = 1 << 20;
// shared empty set for route evaluations that carry no tentative claims
SwimLaneCompactLayout.NO_EXTRA = new Set();
// route shape candidates by the relative position of the endpoints
SwimLaneCompactLayout.SHAPES_SAME_LANE = ['H', 'U', 'D'];
SwimLaneCompactLayout.SHAPES_SAME_COL = ['H'];
SwimLaneCompactLayout.SHAPES_CROSS = ['H', 'V'];
